...
class SummaryDecoder(nn.Module):
    ...
    def forward(self, input_token, hidden_state, document_output, document_mask):
        input_token_embedded = self.embedding(input_token)
        lstm_output, hidden_state = self.lstm(input_token_embedded.unsqueeze(1), hidden_state)

        lstm_output_tiled = lstm_output.expand(-1, document_output.size(1), -1)
        transformed_lstm_output = self.W5(lstm_output_tiled)
        transformed_document_output = self.W6(document_output)
        combined = torch.tanh(transformed_lstm_output + transformed_document_output)
        document_scores = torch.matmul(combined, self.u1)
        document_scores = document_scores.masked_fill(~document_mask.bool(), float('-inf'))
        ...
        combined_context = torch.cat((lstm_output.squeeze(1), document_context_vector), dim=1)
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, target_summary=None, teacher_forcing_ratio=0.5):
        ...
        decoder_input = torch.zeros((batch_size,), dtype=torch.long).to(self.device)
        hidden_state = None
        ...
        for t in range(target_summary.size(1)):
            ...
            decoder_input = target_summary[:, t] if teacher_force else top1
    ...
    def generate_summary(self, input_ids, attention_mask, max_length=50):
        ...
        with torch.no_grad():
            ...
            for _ in range(max_length):
                output, hidden_state = self.decoder(decoder_input, hidden_state, document_output, document_mask)
                top1 = output.argmax(1)
                summary_tokens.append(top1.unsqueeze(1))
                decoder_input = top1
